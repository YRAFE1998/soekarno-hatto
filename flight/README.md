...
```
./mvnw clean package
docker build -t flight:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 flight:v1
```

Once the runtime starts, you can access the project at [http://localhost:8080/flight](http://localhost:8080/flight).
